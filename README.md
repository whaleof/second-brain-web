...
**工程化亮点**：同步合并逻辑抽成纯函数并覆盖 **35 个单元/集成测试**（pytest 25 + node 10，覆盖冲突合并 / tombstone 防复活 / timeline 去重 / schema 校验）；写入层带 schema 校验与明确错误码；前后端通过 `schemaVersion` 对齐；密钥经 `.env` / 环境变量管理，不入库。
